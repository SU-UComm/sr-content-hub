...
const Step = (step) => {
    ...
    return (
        <li className="steps__list-item">
            <button disabled={!canClick} className={stepButtonClassNames} onClick={() => canClick && dispatch({type: 'setCurrentStep', payload: id})}>
                {name}
            </button>
        </li>
    );
    ...
    return (
        <section className="steps">
            <ul className="steps__list">
                {stepsData(window.translations).map((step) => {
